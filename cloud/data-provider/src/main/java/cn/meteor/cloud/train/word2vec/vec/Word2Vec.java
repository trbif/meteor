package cn.meteor.cloud.train.word2vec.vec;

import cn.meteor.cloud.train.word2vec.util.*;
    ...
    private void buildVocabulary() {
        ...
        for (String wordText : wordCounter.keySet()){
            ...
            neuronMap.put(wordText,
                    new WordNeuron(wordText, wordCounter.get(wordText), vectorSize));//构造中添加向量
        }
...
//        System.out.println("共读取了 " + neuronMap.size() + " 个词。");
    ...
    public void training(){
        ...
        //添加词频大于等于freqThresold的单词到WordNeuron
        buildVocabulary();
        //构建哈夫曼树
...
//                System.err.println("临时文件未被正确删除，位于"+tempCorpus.getAbsolutePath());
    ...
    private void skipGram(int index, List<WordNeuron> sentence, int b, double alpha) {
        ...
        WordNeuron word = sentence.get(index);//按照index顺序获取词元
//        if(word.getName().trim().equals("指南针")){
//            double sum = 0;
//            for (int j = 0; j < vectorSize; j++) {
//                sum += word.vector[j]*word.vector[j];
//            }
//            System.out.println("##########word_name::"+word.getName());
//            System.out.println("##########word_sum ::"+sum);
//            
//        }
        ...
        for (a = b; a < windowSize * 2 + 1 - b; a++) {
            ...
            List<HuffmanNode> pathNeurons = word.getPathNeurons();//index词元对应的霍夫曼树的节点，只有一颗霍夫曼树，叶子到顶点的路径（再做倒置），更新了空节点权值，word的权值未变
            WordNeuron we = sentence.get(c);//按照另一种逻辑获取词元，index附近的那几个词元
//            System.out.println(word.getName()+"------"+we.getName());
//            System.out.println(pathNeurons.size());
            for (int neuronIndex = 0; neuronIndex < pathNeurons.size() - 1; neuronIndex++){
                //获取当前词对应的上下文词元，然后在霍夫曼树种查找该词元
                HuffmanNeuron out = (HuffmanNeuron) pathNeurons.get(neuronIndex);
                double f = 0;
                // Propagate hidden -> output
                for (int j = 0; j < vectorSize; j++) {//输入的词元到其上下文中当前一个HuffmanNeuron的cos距离
                    f += we.vector[j] * out.vector[j];
//                    System.out.println("("+we.vector[j]+","+out.vector[j]+")");
                }
                if (f <= -MAX_EXP || f >= MAX_EXP) {//点乘值不在(-6,6)区间则进行下一个HuffmanNeuron距离计算，无需更新权值
//                    System.out.println("F: " + f);
                    continue;
                } else {
                    f = (f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2);//f属于(0,EXP_TABLE_SIZE)
                    f = expTable[(int) f];//算出激活函数对应的值，激活函数本身值域属于(0,1)
                }
                // 'g' is the gradient multiplied by the learning rate
                HuffmanNeuron outNext = (HuffmanNeuron) pathNeurons.get(neuronIndex+1);//使用next是因为从顶点开始，均是在找它的子节点
                double g = (1 - outNext.code - f) * alpha;
                for (c = 0; c < vectorSize; c++) {
                    neu1e[c] += g * out.vector[c];//neu1e用于更新词元向量
                }
                // Learn weights hidden -> output
                for (c = 0; c < vectorSize; c++) {
                    out.vector[c] += g * we.vector[c];//更新空节点向量
                }
            }
            // Learn weights input -> hidden
            for (int j = 0; j < vectorSize; j++) {
                we.vector[j] += neu1e[j];//所有的空节点走完后，更新词元向量，准备切换到下一个非word词元
            }
//            if(we.getName().trim().equals("指南针")){
//                double sum = 0;
//                for (int j = 0; j < vectorSize; j++) {
//                    sum += we.vector[j]*we.vector[j];
//                }
